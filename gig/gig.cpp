...
namespace gig{
  ...
  double ratio_of_uniforms_noshift(double lambda, double lambda_old, double omega,
                                  double alpha, std::mt19937_64 &generator);
  ...
  inline double normal(std::mt19937_64 &generator) {
    return std::normal_distribution<double>()(generator);
  }

  inline double uniform(std::mt19937_64 &generator) {
    return std::uniform_real_distribution<double>()(generator);
  }

  inline double gamma(double shape, double scale, std::mt19937_64 &generator) {
   ...
   * @return        sample
   */
  double GIG_gen::sample(double lambda, double chi, double psi) {
    double omega, alpha;
    double res = NAN;
